...
function updateRefButtonLabels() {
  document.querySelectorAll("[data-ref-call]").forEach(btn => {
    const side = btn.dataset.side;
    const call = btn.dataset.refCall;
...
const count = state.refProgress?.[side]?.[keyMap[call]] || 0;
    const progression = REF_PROGRESSIONS[call] || [];
    const next = progression[count] ?? "DQ";

    let suffix = "";

    if (next === 0) suffix = "W";
    else if (next === "DQ") suffix = "DQ";
    else suffix = `+${next}`;

    // 👇 THIS IS WHERE YOUR LINE GOES
    const sideText = side === "opponent" ? "RED" : "GRN";
    ...
    btn.textContent = `${sideText} ${labelMap[call]} ${suffix}`;
  });
}
/* POSITION GUARD */
